...
int main() {
    ...
    unsigned int woodTexture = loadTexture(getLocalPath("texture/test.jpg").c_str(), true);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, scrWidth, scrHeight, 0, GL_RGBA, GL_FLOAT, NULL); // 16f
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, hdrFBO);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (GLfloat)scrWidth / (GLfloat)scrHeight, 0.1f, 100.0f);
        ...
        for (unsigned int i = 0; i < lightPositions.size(); i++) {
            shader.setVec3("lights[" + std::to_string(long(i)) + "].Position", lightPositions[i]);
            shader.setVec3("lights[" + std::to_string(long(i)) + "].Color", lightColors[i]);
        }
