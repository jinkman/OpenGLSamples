...
vec3 getColor(vec2 uv)
{
    ...
    if(ray.y >= -0.01)
    {
        vec3 C = getatm(ray) * 2.0 + sun(orig,ray);  //天空与太阳颜色
    ...
    vec3 C = fresnel * getatm(R) * 2.0 + fresnel * sun(orig,R);
    C = normalize(C) * sqrt(length(C)); //gamma校正
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i=0;i<AA_SAMPLES;i++){
        C += getColor(uv + vec2(rand2sTimex(uv), rand2sTimex(uv + 100.0)) / iResolution.xy);  //随机扰动   
        uv += 200.0;
        W += 1.0;
    }
    C /= W;
    fragColor = vec4(C,1.0);   //最终颜色
}
void main( void ) 
{
    vec4 color = vec4(0.0);
